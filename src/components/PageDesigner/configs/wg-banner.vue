  ...
  <div>
    <el-form label-position="top">
      <el-form-item label="展示图片(最多添加10个广告，鼠标拖拽调整广告顺序)">
        ...
        <div v-if="selectWg.list.length!=10" class="banner-item btn" @click="showDiglog()">+ 点击添加图片
          <label>还可以上传 {{ 10-selectWg.list.length }} 张</label>
        </div>
      </el-form-item>
      ...
      <el-form-item label="显示滑块">
        <el-switch
          v-model="selectWg.indicator.show"
          active-text="显示"
          inactive-text="隐藏"
        >
        </el-switch>
      </el-form-item>
      <el-form-item label="滑块颜色">
        <el-color-picker v-model="selectWg.indicator.color" />
      </el-form-item>
      <el-form-item label="滑块样式">
        <el-radio v-model="selectWg.indicator.style" :label="0">样式一</el-radio>
        <el-radio v-model="selectWg.indicator.style" :label="1">样式二</el-radio>
        <el-radio v-model="selectWg.indicator.style" :label="2">样式三</el-radio>
      </el-form-item>
    </el-form>
    <!-- 素材选择 -->
    <el-dialog
